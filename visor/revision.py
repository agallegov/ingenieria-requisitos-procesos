...
def rutas_planos(mapa):
    ...
    faltan = [str(ruta) for ruta in rutas if not ruta.is_file()]
    if faltan:
        raise ValueError("faltan planos de actividades: %s" % ", ".join(faltan))
    return rutas
...
def ejecutar_validador(mapa, perfil="revision"):
    r = subprocess.run(
        [
            sys.executable,
            str(BASE / "validar.py"),
