...
(investigación, planificación, especificación, construcción, cierre):
    ...
    <destino>/            meta-repo (git propio, con commit inicial)
      ...
      docs/00-metodo/     el método íntegro: 12 runbooks, 11 plantillas, sus scripts
...
Uso:
  python RUTA_HERRAMIENTA/visor/bootstrap.py \
...
METODO_RAIZ = (
    ...
    "auditoria-seguridad.md", "seguridad-por-stack.md", "sandbox.md",
)
...
def generar_ci():
    return (
        ...
        "      - run: python docs/00-metodo/scripts/lint_metodo.py\n"
    )
...
def proteger_main(owner, nombre):
    ...
    cuerpo = ('{"required_status_checks":{"strict":true,"contexts":["tests"]},'
...
def montar_git(destino, nombre_codigo, titulo, remoto_codigo, remoto_meta):
    ...
    if remoto_codigo and not remoto_esta_vacio(remoto_codigo):
        ...
        # El repo de código nace LIMPIO: README + CI. Todo lo de agentes y método vive
        # en el padre; aquí solo irá el código de la aplicación (la CI es infra del repo).
        ...
        (flujos / "tests.yml").write_text(
            ...
            "      - name: Lint (KISS: funciones cortas, sin duplicación evidente)\n"
            "        run: |\n"
            "          if ls pyproject.toml ruff.toml .ruff.toml 2>/dev/null | grep -q .; then\n"
            "            pip install -q ruff && ruff check .\n"
            "          else\n"
            "            echo 'Sin config de ruff aún: llega con el esqueleto.'\n"
            "          fi\n"
            "      - name: Suite\n"
            "        run: |\n"
...
def main():
    ...
    # registrar después con `python visor/proyectos.py registrar RUTA`.
