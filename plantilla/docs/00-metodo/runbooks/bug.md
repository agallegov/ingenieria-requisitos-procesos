...
bug "micro": todo bug lleva ficha, `NNN`, test rojo y OK del usuario. Si al triarlo parece
demasiado pequeño para una ficha, sigue siendo un bug — el tamaño del diff no es criterio de
carril (`00-metodo/README.md`, los 3 carriles). Un bug que revele un problema de diseño
transversal → escalar a decisión con el usuario.
...
1. **Reporte.** El usuario se lo cuenta al agente en la terminal (rol CONSTRUCTOR, según el
   ...
   `python docs/00-metodo/scripts/unidad.py nueva bug <slug>` (asigna el siguiente NNN desde
...
2. **Despacho.** `python docs/00-metodo/scripts/unidad.py despachar NNN-slug`: crea la rama
...
3. **Reproducir (primera misión del subagente).** Crear un test end-to-end que reproduzca el
   ...
   aquí.)
4. **Diagnosticar.** La causa raíz, al fichero del bug (siempre).
...
9. **Cierre (el padre, a petición del usuario).** Es el ritual indivisible de
   ...
   Después, **revisor fresco** (sesión/subagente nuevo, solo lectura): *"Revisa el diff contra
