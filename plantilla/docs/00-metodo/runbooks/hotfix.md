...
2. **Ficha y despacho, a la vez (el padre).** `unidad.py nueva bug <slug>` (ficha desde
   `plantillas/bug.md`), severidad P0 y la marca de deuda al principio, alta en el índice; acto
   seguido `unidad.py despachar NNN-slug --force` — **el único uso legítimo de `--force`**, que
   salta la precondición del contrato escrito y deja la deuda anotada. Estado → `en_obra`,
   subagente lanzado, `ESTADO.md` actualizado.
