...
| 7 | Cierre | unidad | verificar → revisar → merge → e2e + instancia validada → consolidar (indivisible) |
...
sea): 1. verificar con
evidencia (checks + output) · 2. revisor fresco: diff contra especificación (requisitos, edge
cases, alcance; **que los ficheros de test no se tocaron después del commit que los creó** y
**que no se ha duplicado un módulo que ya existía en `main/`**) — veredicto a la sección
**Revisión** de `hallazgos.md`; huecos → vuelven al
constructor · 3. merge · 4. **suite end-to-end completa sobre main** · 5. **lanzar una
instancia de la app** (comando de arranque: el `AGENTS.md` del repo de código) y **validación
del usuario sobre ella**; sin su OK no hay cierre, "no es lo que pedí" → unidad `bug` ·
6. aplicar deltas declarados a `02-flujos/` · 7. promover hallazgos → `conocimiento/`,
decisiones → `decisiones/` · 8. actualizar `ESTADO.md` + `INDICE.md` · 9. mover unidad a
`archivo/` — los bugs no se archivan: su fichero se queda en `docs/bugs/` (ADR-006) ·
10. borrar worktree y rama. Las puertas verificables del despacho y la coherencia estructural
las aplican `unidad.py`, el hook pre-push y `lint_metodo.py`; las pruebas de la aplicación,
la revisión y el OK humano dependen de los comandos reales documentados en el repo de código.

## Piezas del método

- `runbooks/` — además de los 7 tipos: `expres.md` (carril exprés: rama efímera, sin unidad)
  y `hotfix.md` (producción caída: se salta la espera de aprobación, con deuda de spec a
  pagar en 24 h). Cada tipo tiene un recorrido completo, sus puertas y su cierre.
...
- `plantillas/` — por unidad: especificacion (contrato + plan de trabajo, ADR-005),
  ...
  por proyecto: informe y sintesis (fase 3), roadmap (fase 4).
...
- `scripts/lint_metodo.py` — **el linter del método**: valida estructura congelada,
  ...
  ESTADO ≤100, coherencia worktrees↔unidades, cierres a
  medias. Se ejecuta al arrancar sesión del padre, al final de cada cierre, y en CI.
