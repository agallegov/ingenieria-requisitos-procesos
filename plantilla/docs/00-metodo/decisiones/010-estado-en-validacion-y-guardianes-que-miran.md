...
3. **El entorno del worktree se prepara al crearlo.** Un worktree recién creado es código sin
   ...
   método no sabe montar eso —depende del stack— pero sí sabe cuándo hay que montarlo: si el
   proyecto deja un `worktree-listo` (o `worktree-listo.py`) en la raíz del meta-repo,
   `unidad.py despachar` lo ejecuta con el worktree recién creado. Es del proyecto, no del
   método: ninguna actualización del método lo pisa.
